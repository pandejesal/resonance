    ...
    <div className="space-y-6">
      ...
      {!hasAdvancedAnalytics && (
        <div className="surface-card p-6 text-center">
          <div className="w-12 h-12 mx-auto mb-3 rounded-full bg-brand-500/10 flex items-center justify-center">
            <svg className="w-6 h-6 text-brand-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
            </svg>
          </div>
          <h3 className="text-sm font-bold text-primary mb-1">Advanced Analytics</h3>
          <p className="text-xs text-secondary mb-3">Upgrade to Pro for detailed listening insights</p>
      ...
      {/* Top artists */}
      {stats.top_artists.length > 0 && hasAdvancedAnalytics && (
        <section>
      ...
      {/* Most played */}
      {stats.most_played.length > 0 && hasAdvancedAnalytics && (
        <section>
