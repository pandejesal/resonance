...
pub async fn match_tracks(pool: &SqlitePool, preview: &mut ImportPreview) {
    ...
    for import_track in preview.unmatched.drain(..) {
        ...
        if !norm_title.is_empty() && !norm_artist.is_empty() {
            if let Some(cands) = by_pair.get(&(norm_title.clone(), norm_artist.clone())) {
                if let Some((id, _, _)) = cands.first() {
                    best_match = Some((id.clone(), "exact".to_string(), 1.0));
                }
            } else if let Some(cands) = by_title.get(&norm_title) {
                let pick = cands
                    .iter()
                    .find(|(_, _, cand_artist)| cand_artist == &norm_artist)
                    .or_else(|| cands.first());
                if let Some((id, _, _)) = pick {
