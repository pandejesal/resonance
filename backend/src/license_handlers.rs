...
pub async fn activate_license(
    ...
    if !check_rate_limit(&format!("license_activate:{}", ip), 10, 60) {
        return HttpResponse::TooManyRequests().json(serde_json::json!({"error": "Too many requests. Try again later."}));
    }
    let key = body.license_key.trim();
    if key.is_empty() || key.len() > 64 {
        return HttpResponse::BadRequest().json(serde_json::json!({"error": "Invalid license key format"}));
    }
...
pub async fn deactivate_license(
    ...
    let license = match License::get_by_key(&data.db, &body.license_key).await {
        Some(lic) => lic,
        None => return HttpResponse::BadRequest().json(serde_json::json!({"error": "Invalid license key"})),
    };
    if license.user_id.as_deref() != Some(&user.id) {
        return HttpResponse::Forbidden().json(serde_json::json!({"error": "You can only deactivate your own license"}));
    }
...
pub async fn generate_license_key(
    ...
    if !matches!(tier.as_str(), "pro" | "enterprise") {
        return HttpResponse::BadRequest().json(serde_json::json!({"error": "Invalid tier. Must be 'pro' or 'enterprise'."}));
    }
    ...
    match result {
        ...
        Err(e) => HttpResponse::InternalServerError().json(serde_json::json!({"error": format!("Failed to generate license key: {}", e)})),
    }
}
