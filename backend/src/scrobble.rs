...
impl ScrobbleService {
    ...
    pub async fn test_lastfm_connection(
        ...
        if status.is_success() {
            ...
            if let Some(name) = body.pointer("/user/name").and_then(|n| n.as_str()) {
    ...
    pub async fn test_listenbrainz_connection(
        &self,
        config: &ListenbrainzConfig,
    ) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let token = config
        ...
        if status.is_success() {
            ...
            return Err(format!("ListenBrainz rejected token: {}", message).into());
        }

        Err(format!("ListenBrainz HTTP {}: {}", status, body_text).into())
    }
...
pub(crate) async fn set_setting(db: &SqlitePool, key: &str, value: &str) {
    sqlx::query(
